...
class iteration:
    def __init__(self, tensor_shape, dim,M,X_train):
        ...
        for dd in range(dim):
            ...
            self.cur_basis.append( tensor(dim,dd).compute_range_basis(self.X_train, self.tensor_shape) )
    ...
    def iteration_compute_P_given_index(self, index):
        ...
        for i in range(len(self.X_train)):
            basis_val=generate_basis_vector().compute_basis_val_new_coordinates(self.dim, self.X_train[i], self.cur_shape, new_coordinates)
    ...
    def compute(self):
        ...
        for index in range(self.dim):
            print('iteration oordinate =', index)
